...
from zim_to_corpus.html import headerp, get_section_title_tag
...
def remove_sections(bs: BeautifulSoup,
    ...
    def post_remove(_, tag):
        if tag.name == 'section':
            try:
                title_tag = get_section_title_tag(tag)
            ...
            if title_tag.name != 'h1':
                if sections and title in sections:
                    ...
                    tag.decompose()
                elif regex and regex.search(title):
                    logging.debug(f'Removing title {title} by regex...')
                    tag.decompose()
                elif is_empty(tag):
